...
export interface DatosTarjetaEmprendedor {
  imagenUrl: string;
...
// densidad (mismo criterio que tarjetaRecorrido.ts/tarjetaPost.ts).
const ESCALA = 2;
...
// generarTarjetaCompartirEmprendedor) para mostrar el título y la
...
// Genera la vista previa que se comparte junto con la ficha de emprendedor
// (imagen + nombre del negocio + descripción + logo de Legión Roller) — así
// el destinatario ve de qué trata sin depender de que la red social muestre
// el texto del `share` nativo por separado (mismo patrón que tarjetaPost.ts).
export async function generarTarjetaCompartirEmprendedor(datos: DatosTarjetaEmprendedor): Promise<Blob> {
  ...
  if (!fotoDataUrl) throw new Error("No se pudo cargar la imagen de la ficha");
  ...
  const lineasTitulo = envolverTexto(medidor, datos.nombreNegocio.trim(), anchoTexto);

  medidor.font = `400 ${RESENA_FONT}px Arial, sans-serif`;
  const descripcion = datos.descripcion.trim();
  const lineasResena = descripcion ? envolverTexto(medidor, descripcion, anchoTexto) : [];
  ...
  const svg = `
    <svg width="${ANCHO * ESCALA}" height="${alto * ESCALA}" viewBox="0 0 ${ANCHO} ${alto}" xmlns="http://www.w3.org/2000/svg">
      <clipPath id="marcoTarjetaEmprendedor">
        <rect width="${ANCHO}" height="${alto}" rx="26"/>
      </clipPath>
      <g clip-path="url(#marcoTarjetaEmprendedor)">
