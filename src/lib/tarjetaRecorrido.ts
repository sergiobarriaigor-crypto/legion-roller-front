  ...
  const statsSvg = stats
    .map((s, i) => {
      ...
      return `
        ...
        <text x="${cx}" y="${etiquetaY}" text-anchor="middle" font-family="Arial, sans-serif" font-size="15" font-weight="600" letter-spacing="0.5" fill="${GRIS_TEXTO}">${escapeXml(s.etiqueta)}</text>
      `;
    })
    .join("");
