...
// de degradado que Mis Rutas), hasta 5 destinatarios por acción.
export function SelectorCompartirEmprendedor({
  emprendedor,
  propioId,
  token,
  onCerrar,
  ...
  async function enviar() {
    ...
    setError("");
    try {
      await compartirEmprendedorAUsuarios(emprendedor.id, seleccionados, token);
      onCerrar();
    } catch {
      setError("No se pudo compartir la ficha. Probá de nuevo.");
      setEnviando(false);
    }
  }
