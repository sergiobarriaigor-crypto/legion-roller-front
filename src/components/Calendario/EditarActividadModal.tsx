...
export function EditarActividadModal({
  actividadId,
  propioId,
  token,
  onGuardada,
  ...
  useEffect(() => {
    obtenerActividad(actividadId, token)
      .then((a) => {
        setCategoria(a.categoria as CategoriaActividad);
        setTitulo(a.titulo);
        setDescripcion(a.descripcion ?? "");
        setFecha(a.fecha);
        setHora(a.hora ?? "");
        setPuntoEncuentro(a.puntoEncuentro ?? "");
        setInvitadosIds(a.invitadosIds);
        setMinutosAvisoCreador(a.minutosAvisoCreador);
        setFotoUrl(a.fotoUrl);
        setMusicaId(a.musicaId);
      })
  ...
  async function guardar() {
    ...
    try {
      ...
      setError("No se pudo guardar los cambios. Probá de nuevo.");
      setGuardando(false);
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-6" onClick={onCerrar}>
      ...
      >
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-semibold text-text-accent">Editar actividad</h2>
